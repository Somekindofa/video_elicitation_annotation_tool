#!/usr/bin/env python3
"""
Migration script to add is_salient field to annotations table.
This field marks salient moments for apprentice learning.
"""
...
def main() -> int:
    ...
    logger.info("Starting migration: adding is_salient field")
    ...
    if CHROMA_DB_PATH.exists():
        ...
        if not add_column_if_not_exists(
            CHROMA_DB_PATH, "annotations", "is_salient", column_def
        ):
            success = False
    ...
    if ELICITATIONS_DB_PATH.exists():
        ...
        if not add_column_if_not_exists(
            ELICITATIONS_DB_PATH, "annotations", "is_salient", column_def
        ):
            success = False
